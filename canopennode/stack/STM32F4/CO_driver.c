...
/* Private variable ----------------------------------------------------------*/
/* Private function ----------------------------------------------------------*/
static uint8_t CO_CANClkSetting (void);
static uint8_t CO_CANconfigGPIO (void);
        ...
        uint16_t                CANbitRate)
{
    ...
    CAN_ITConfig(CANmodule->CANdriverState, (CAN_IT_TME | CAN_IT_FMP0 | CAN_IT_FMP1), DISABLE);
    ...
    result = CO_CANClkSetting();

    /* GPIO Config for CAN */
    result = CO_CANconfigGPIO();
	...
	CAN_InitStructure.CAN_NART = ENABLE;		// ENABLED = resending disabled
	CAN_InitStructure.CAN_RFLM = DISABLE;
	CAN_InitStructure.CAN_TXFP = DISABLE;
	CAN_InitStructure.CAN_Mode = CAN_Mode_Normal;

    result = CAN_Init(CANmodule->CANdriverState, &CAN_InitStructure);
    if (result == 0) {
    ...
    NVIC_InitStructure.NVIC_IRQChannel = CAN2_RX0_INTERRUPT;
    NVIC_Init(&NVIC_InitStructure);
    NVIC_InitStructure.NVIC_IRQChannel = CAN2_RX1_INTERRUPT;
    NVIC_Init(&NVIC_InitStructure);
    /* interrupts from transmitter */
    NVIC_InitStructure.NVIC_IRQChannel = CAN2_TX_INTERRUPTS;
    NVIC_Init(&NVIC_InitStructure);

    /* Can_init function of ST Driver puts the controller into the normal mode */

    CAN_ITConfig(CANmodule->CANdriverState, (CAN_IT_TME | CAN_IT_FMP0 | CAN_IT_FMP1), ENABLE);
...
void CO_CANverifyErrors(CO_CANmodule_t *CANmodule)
{
    ...
    if(CANmodule->errOld != err) {
        ...
        /* CAN RX bus overflow */
		if(CANmodule->CANdriverState->RF1R & 0x10) {
			CO_errorReport(em, CO_EM_CAN_RXB_OVERFLOW, CO_EMC_CAN_OVERRUN, err);
			CANmodule->CANdriverState->RF1R &=~0x10;//clear bits
		}
...
/******************************************************************************/
/* Interrupt from receiver, FIFO0 */
void CO_CANinterrupt_Rx0(CO_CANmodule_t *CANmodule)
{
    CanRxMsg CANx_RxMsg;
    uint16_t index;
	...
	CAN_Receive(CANmodule->CANdriverState, CAN_FilterFIFO0, &CANx_RxMsg);

    for (index = 0; index < CANmodule->rxSize; index++) {
        uint16_t msg = (CANx_RxMsg.StdId << 2) | (CANx_RxMsg.RTR ? 2 : 0);
    ...
    if (msgMatched && msgBuff->pFunct) {
        msgBuff->pFunct(msgBuff->object, (CO_CANrxMsg_t*) &CANx_RxMsg);
    }
}

/* Interrupt from receiver, FIFO1 */
void CO_CANinterrupt_Rx1(CO_CANmodule_t *CANmodule)
{
    CanRxMsg CANx_RxMsg;
    uint16_t index;
    uint8_t msgMatched = 0;
	...
	CAN_Receive(CANmodule->CANdriverState, CAN_FilterFIFO1, &CANx_RxMsg);
...
static uint8_t CO_CANsendToModule(CO_CANmodule_t *CANmodule, CO_CANtx_t *buffer)
{
    ...
    txMbox->TIR &= 1; 									// TMIDxR_TXRQ = 0
    txMbox->TIR |= ((buffer->ident) | CAN_RTR_DATA);	// |= ((TxMessage->StdId << 21) | TxMessage->RTR);
    ...
    txMbox->TIR |= 1;									// TMIDxR_TXRQ = 1

    CAN_ITConfig(CANmodule->CANdriverState, CAN_IT_TME, ENABLE); // Transmit mailbox empty interrupt

    return 0;
}

/******************************************************************************/
uint8_t CO_CANClkSetting (void)
{
	volatile uint8_t ret = CO_ERROR_NO;

	RCC_AHB1PeriphClockCmd(CAN_GPIO_CLK, ENABLE);			// GPIO
	RCC_APB1PeriphClockCmd(CAN_CLK, ENABLE);				// CAN2
	RCC_APB1PeriphClockCmd(CAN_MASTER_CLK, ENABLE);			// CAN1
